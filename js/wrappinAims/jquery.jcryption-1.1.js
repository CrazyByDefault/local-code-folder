(function(A) {
    ...
    A.jCryption.getKeys = function(B, D) {
        var C = this;
        C.getKeys = function() {
            if (encryptionDefMode == encrApplicationMode) {
                keys = new C.jCryptionKeyPair(B.e, B.n, B.maxdigits);
            } else {
                if (encryptionDefMode == encrSessionMode) {
                    A.ajax({
                        url: B,
                        dataType: "json",
                        type: "POST",
                        success: function(E) {
                            keys = new C.jCryptionKeyPair(E.e, E.n, E.maxdigits);
                            if (A.isFunction(D)) {
                                D.call(this, keys);
                            }
                        }
        ...
        C.jCryptionKeyPair = function(G, E, F) {
            setMaxDigits(parseInt(F, 10));
            this.e = biFromHex(G);
            this.m = biFromHex(E);
    ...
    A.jCryption.encrypt = function(H, D, K) {
        var C = 0;
        ...
        var E = "";
        E += L.charAt((C & 240) >> 4) + L.charAt(C & 15);
        var B = E + H;
        var I = [];
        var F = 0;
        while (F < B.length) {
        ...
        function J(R) {
            ...
            function M() {
                Q = new BigInt();
                N = 0;
                for (var S = O; S < O + D.chunkSize; ++N) {
                    Q.digits[N] = R[S++];
                    Q.digits[N] += R[S++] << 8;
                }
                var U = D.barrett.powMod(Q, D.e);
                var V = D.radix == 16 ? biToHex(U) : biToString(U, D.radix);
                P += V + " ";
                O += D.chunkSize;
                if (O < R.length) {
                    setTimeout(M, 1);
                } else {
                    var T = P.substring(0, P.length - 1);
                    if (A.isFunction(K)) {
                        K(T);
                    } else {
                        return T;
